# Volume 3 — ADK 2.x Workflow Engineering for Production Agents

> [!CAUTION]
> **Status: Draft — not approved for production use.** Re-researched 2 August 2026 against ADK Python v2.6.1, pinned Google source, and current Gemini Enterprise Agent Platform documentation. Twenty-six dependency-free tests pass across domain, evaluation, delivery, and qualification gates, and the real graph compiles against a temporary resolved v2.6.1 installation. Customer cloud, online evaluation, managed-session, load, recovery, security, and independent review gates remain open. See [content status](../STATUS.md) and the [implementation evidence ledger](../../references/implementation/volume-3-adk.md).
...
ADK Python 2.0 reached general availability on 19 May 2026. This repository qualifies v2.6.1, released 31 July 2026. Workflow APIs can evolve in later minor releases. Pin the exact version qualified by your platform team, run compatibility tests before upgrades, and review release notes as part of every promotion.

### 2.5 Executable companion and evidence boundary

The tested implementation is under [examples/python/adk-enterprise-workflow](../../examples/python/adk-enterprise-workflow/README.md), with [delivery](../../delivery/volume-3-adk/README.md), [field labs](../../labs/volume-3-adk/README.md), [operations](../../operations/volume-3-adk/README.md), and an [exact evidence ledger](../../references/implementation/volume-3-adk.md). Inline snippets in this chapter explain patterns; they are not all standalone programs. Only files named by the companion README and CI workflow are claimed as executable gates.

### 2.6 Primary official references
...
- Develop and deploy an ADK agent on Agent Runtime: <https://docs.cloud.google.com/gemini-enterprise-agent-platform/build/runtime/quickstart-adk>
- Agent Runtime deployment methods: <https://docs.cloud.google.com/gemini-enterprise-agent-platform/scale/runtime/deploy-an-agent>
...
The executable companion pins the qualified direct dependencies as follows. A customer build must additionally generate and archive a hash-locked transitive dependency set; exact direct pins alone are not a complete lock.
...
**Production action:** qualify the exact release approved by the customer, regenerate a hash-locked transitive constraints file, scan it, and rerun graph, resume, migration, evaluation, and rollback tests before promotion.
...
This syntax was checked against the v2.6.1 public exports and official route sample. The production principle is stable: routing is returned as data, not hidden in prompt text.
...
The constructor and collaboration modes remain version-qualified interfaces. Use an exact, customer-qualified model ID in production configuration; a convenience alias such as `gemini-flash-latest` is useful for exploration but is not an immutable production model pin.
...
### 12.3 Version-specific task-mode correction

The imported v2.0-era warning that task mode was disabled inside graph workflows is no longer current for the qualified v2.6.1 source. The workflow scheduler contains explicit task-mode waiting and isolation handling. This is still a version-sensitive behavior: prove clarification, interruption, isolation, resume, cancellation, and timeout paths before using task-mode agents in a customer workflow. The immutable source evidence is recorded in the implementation ledger.
...
release:
  ...
  release_id: 2026.08.02-rc1
  source_commit: REPLACE_WITH_IMMUTABLE_REVISION
  adk_version: 2.6.1
  ...
  model:
    planner: CUSTOMER_QUALIFIED_EXACT_MODEL_ID
    workers: CUSTOMER_QUALIFIED_EXACT_MODEL_ID
    verifier: CUSTOMER_QUALIFIED_EXACT_MODEL_ID
  tool_contracts:
    case_api: v2
    policy_search: v1
  evalset_version: 17
...
ADK supports evaluation through `adk web`, programmatic tests, `adk eval`, and conformance testing. The companion uses hermetic domain evaluations on every change and reserves paid/model-judged criteria for an authenticated customer project with approved data and budgets.
...
adk eval \
  path/to/agent_package \
  path/to/customer-approved.evalset.json \
  --config_file_path=evals/adk_eval_config.json \
  --print_detailed_results
```

Run the dependency-free deterministic gate first. Online groundedness and safety criteria require approved cloud authentication, data, cost, and evidence retention; they must not silently fall back to a skipped or empty result.
...
The production target is a graph that resembles the following. The executable companion implements the safe control slice; customer-specific model, approval, ledger, and tool adapters are integration work, not generic defaults.
...
def build_root_workflow() -> Workflow:
    return Workflow(
        ...
        edges=[
            ...
            (policy_agent, join_node),
            (risk_agent, join_node),
            (data_node, join_node),
...
This fan-out/join form follows the qualified v2.6.1 graph route and `JoinNode` APIs. Every branch must emit an output or the join cannot complete. Preserve the topology behind graph-compilation, output, partial-failure, timeout, and resume tests.
...
The [implementation evidence ledger](../../references/implementation/volume-3-adk.md) maps every implemented decision to current documentation and immutable v2.6.1 source/sample URLs. The catalogue below is a broader reading list.
...
## 46. Implemented deliverable and next chapter

This volume now includes a production-shaped enterprise change workflow, deterministic contracts, authorization and approval controls, idempotent tool boundary, unknown-outcome reconciliation, audit redaction, evaluation cases, guarded Agent Runtime qualification deployment, CI, field labs, and incident runbooks. It deliberately does not pretend that an in-memory test ledger or a documentation citation proves customer production readiness.

**Chapter 4 — Building the Enterprise Execution Loop** can extend this foundation with a customer-selected durable ledger, authenticated approval provider, real tool adapter, managed-session migration, event adapters, infrastructure, online model evaluation, and sandbox evidence. Those integrations require customer architecture and authority; they cannot be safely fabricated as generic defaults.
