...
class QualificationTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls): cls.gates = json.loads((HERE / "required-gates.json").read_text())

    def test_examples_are_not_production_qualified(self):
        for volume in self.gates["volumes"]:
            record = json.loads((ROOT / "labs" / f"volume-{volume}" / "qualification.example.json").read_text())
    ...
    def test_complete_records_pass(self):
        for volume, names in self.gates["volumes"].items():
            record = {"schema_version": 1, "volume": volume, "environment": "production",
                      "project_id": "customer-agents-123", "location": "global",
